...
| Profile | `StudentProfile`, `PriorDegree`, `AcademicResult`, `LanguageQualification`, `WorkExperience`, `StudyPreference` | Structured values include grading system/unit and unknown state; history or last-updated metadata supports reproducibility. |
| Catalogue | `University`, `Campus`, `Programme`, `ProgrammeVersion`, `ProgrammeDiscipline`, `Intake`, `ApplicationRoute` | Public reads target an approved immutable programme version. |
| Provenance | `SourceReference`, `VerificationRecord`, `Requirement`, `RequirementRule`, `Deadline` | Requirements/deadlines link to official sources; rules are typed and versioned. |
| Saved | `SavedProgramme` | Unique per user/programme. |
...
| Applications | `Application`, `ApplicationStatusHistory`, `ApplicationNote` | Notes must not invite passwords or excessive sensitive data. |
| Checklist/Documents | `Checklist`, `ChecklistItem`, `ChecklistItemStatusHistory`, `DocumentMetadata`, `DocumentAccessEvent` | `DocumentMetadata` exists only if uploads are enabled; object content is external/private. |
| Notifications | `NotificationPreference`, `DeadlineReminder`, `NotificationMessage`, `NotificationDelivery`, `OutboxMessage` | Preferences and idempotent delivery are first-class. |
| Admin | `ContentRevision`, `ReviewDecision` | Enforces author/reviewer separation and publication history. |
| Privacy | `NoticeVersion`, `ConsentRecord`, `PrivacyRequest`, `DataRetentionAction` | Consent is versioned and withdrawal is preserved as history. |
| Audit | `AuditEvent` | Append-only metadata: actor, action, target type/id, time, outcome, correlation/IP/security context with minimisation/redaction. |
